...
//    });
    ...
    $("form#invite-a-friend-form").submit(function(e) {
        $.post(this.action, $(this).serialize(), function(data) {
            var heading = data.status ? "Friend Invite sent successfully!" : "Friend Invite failed!";
            if(data.status) {
                $("div#notification-container").removeClass('alert-error');
                $("div#notification-container").addClass('alert-success');
            }
            ...
            $.closeDialogWindow();
                ...
                $("div#notification-container h4").html("Error!");
                $("div#notification-container p").html("Error occured while trying to send friend invite. Please try again.");
                $("div#notification-container").removeClass('alert-success');
                $("div#notification-container").addClass('alert-error');
                $("div#notification-container").showNotification();
            });
    ...
    $("#advertizment-container").animateImage();
});
...
$.fn.animateImage = function(){
    ...
    $("#advertizment-container").fadeOut(2000, function(){
        if($('#advertizment-container img').size()*1 > 0){
            var last_id = $('#advertizment-container').children().last().find("img").attr("id").substring(2)* 1;
            var current_id= $('#advertizment-container img.active').attr("id").substring(2)* 1;
            var next_id = current_id >= last_id ? 1 : current_id + 1;
            $("#BN" + current_id).removeClass("active");
            $("#BN" + current_id).addClass("inactive-banner");
            $("#BN" + next_id).removeClass("inactive-banner");
            $("#BN" + next_id).addClass("active");
        }
    }).fadeIn(1000);

    setTimeout( $("#advertizment-container").animateImage, 10000);
}